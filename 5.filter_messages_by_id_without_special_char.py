...
def process_messages(input_file):
    ...
    output_dir = "messages_by_group_id"
    ...
    for i in range(0, total_messages, chunk_size):
        ...
        for message in chunk:
            ...
            if group_id is None:
                ...
                group_files[group_id].append(message)

    # Gravando os arquivos por grupo
    for group_id, messages in group_files.items():
        sanitized_name = sanitize_filename(group_id)
        output_file = os.path.join(output_dir, f"{sanitized_name}.json")
        with open(output_file, 'w', encoding='utf-8') as outfile:
            json.dump(messages, outfile, ensure_ascii=False, indent=4)

    # Exibindo o número de mensagens com group_id igual a null
    print(f"Quantidade de mensagens com 'group_id' igual a null: {null_group_count}")
